...
\begin{stmt}
    ...
    \[
        V \cong H \oplus U,
    \]
    где $B\big|_H$ --- гиперболическая форма, а $U$ --- анизотропное подпространство (то есть все ненулевые векторы в
    $U$ анизотропны).
    ...
    \begin{proof}
        ...
        заменим $\bar{v}'$ на $\bar{v}' - \frac{B(\bar{v}', \bar{v}')}{2} v$. Тогда $B(v, \bar{v}') = 1$ сохраняется,
        а $B(\bar{v}', \bar{v}') = 0$.
...
\begin{lemma}
    Пусть $v, u \in V$ --- анизотропные, $B(v, v) = B(u, u)$, $B$ невырожденна. Тогда $\exists\, e\colon u = \sigma_e(v)$ или
    $-u = \sigma_e(v)$.
    ...
    \begin{proof}
        ...
        Тогда $v+u$ и $v-u$ --- базис $\langle v, u \rangle$, и матрица Грама в этом базисе нулевая. Но тогда
        $B(v, v) = 0$ --- противоречие с анизотропностью $v$.
...
\begin{theorem}
    ...
    \[
        \sigma_e(\sigma_{e'}(v)) = \sigma_e\!\left(v - \frac{2\langle v, e' \rangle}{\langle e', e' \rangle}
        \, e'\right) =
        v - \frac{2\langle v, e' \rangle}{\langle e', e' \rangle}\, e' -
        \frac{2\left\langle v - \frac{2\langle v, e' \rangle}{\langle e', e' \rangle}\, e',\, e\right\rangle}
        {\langle e, e \rangle}\, e.
    \]
...
\begin{definition}
    ...
    если существует базис $e_1, \dots, e_n$ пространства $V$ и однородный многочлен $q(x_1, \dots, x_n)$ степени
    $2$, такой что
    \[
...
\begin{definition}[Матрица Грама квадратичной формы]
    ...
    \[
        ...
        \qquad q(\alpha_1, \dots, \alpha_n) = \sum_{i,j} a_{ij}\, \alpha_i \alpha_j = \sum_{i,j} b_{ij}
        \, \alpha_i \alpha_j.
    \]
\end{definition}
...
\begin{stmt}[Биекция между симметричными билинейными и квадратичными формами]
    ...
    \begin{proof}
        ...
        \[
            \frac{Q(v+u) - Q(v) - Q(u)}{2} = \frac{B(v+u,\, v+u) - B(v,v) - B(u,u)}{2} = \frac{2B(v,u)
            }{2} = B(v,u).
        \]
        Обратная композиция:
        \[
            \frac{Q(v + u) - Q(v) - Q(u)}{2}\bigg|_{u = v} = \frac{Q(2v) - 2Q(v)}{2} = \frac{4Q(v) - 2Q(v)
            }{2} = Q(v).
        \]
    \end{proof}
\end{stmt}
...
\begin{definition}
    Квадратичная форма $Q$ называется \emph{невырожденной}, если соответствующая симметричная билинейная форма
    $B$ невырожденна.
\end{definition}
