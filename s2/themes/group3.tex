...
\begin{remark}
    Действие здесь левое (умножаем на $h$ слева), но класс называется \emph{правым}, так как элемент $g$ стоит \emph{справа}.
\end{remark}
...
\begin{remark}
    ...
    \[
        \varphi_{h_1 h_2}(g) = g h_1 h_2, \qquad (\varphi_{h_1} \circ \varphi_{h_2})(g) = \varphi_{h_1} (g h_2) = g h_2 h_1,
    \]
...
\begin{stmt}
    ...
    \begin{proof}
        ...
        \textbf{Симметричность.} Если $g_1 \sim_L g_2$, то $g_1, g_2 \in Hg$, а значит и $g_2, g_1 \in Hg$, то есть $g_2 \sim_L g_1$.
        ...
        Из $g_1 \in Hg$ следует $g_1 = h_1 g$, а из $g_2 \in Hg$ следует $g_2 = h_2 g$, откуда $g = h_2^{-1} g_2$. Подставляя:
        \[
...
\begin{cor}
    Пусть $G$ --- конечная группа, $g \in G$. Тогда $\mathrm{ord}(g) = min \{n \in \mathbb{N} | g^n = e\}$ делит $|G|$, или считается равной бесконечности.
...
\begin{stmt}
    ...
    \begin{proof}
        \textbf{($\Leftarrow$) Корректность.} Пусть $g_1, t_1 \in g_1 H = t_1 H$ и $g_2, t_2 \in g_2 H = t_2 H$. Покажем, что $t_1 t_2 H = g_1 g_2 H$.
        ...
        \textbf{($\Rightarrow$)} Рассмотрим отображение $\pi\colon G \to G/H$, $g \mapsto gH$. Если $(*)$ задаёт операцию группы, то $\pi$ --- гомоморфизм:
        \[
...
\begin{stmt}[Универсальное свойство фактор-группы]
    ...
    \begin{proof}
        ...
        \textbf{Единственность.} Любой $\psi\colon G/H \to T$, решающий задачу, обязан удовлетворять $\psi(gH) = \psi(\pi(g)) = f(g)$. Значит, $\psi = \bar{f}$.
    \end{proof}
\end{stmt}
...
\begin{stmt}
    ...
    \begin{proof}
        Достаточно показать, что $g [t_1, t_2] g^{-1} \in [G, G]$ для любых $g, t_1, t_2 \in G$ (тогда сопряжение произвольного произведения коммутаторов разбивается на сопряжения каждого множителя).
...
\subsection*{Знакочередующая группа}
 
\begin{definition}
    \emph{Знакочередующая группа} $A_n \coloneqq \ker(\mathrm{sgn}) \subset S_n$ --- подгруппа чётных перестановок.
\end{definition}
...
\begin{lemma}
    ...
    \begin{proof}
        ...
        \]
        Так как $\mathrm{sgn}(\sigma) = 1 = (-1)^N$, число $N$ чётно. Разобьём транспозиции попарно и покажем, что произведение любых двух транспозиций лежит в $T$.
...
\begin{theorem}
    ...
    \begin{proof}
        ...
        Для обратного включения: по утверждению выше для любых трёх элементов $\{i,j,k\} \subset \{1,\ldots,n\}$ группа $S_{\{i,j,k\}} \cong S_3$ вкладывается в $S_n$ как подгруппа, переставляющая только $i,j,k$. По утверждению ниже коммутант $S_3$ --- это $A_3$, состоящий ровно из двух 3-циклов на $\{i,j,k\}$. Значит, все 3-циклы лежат в $[S_n, S_n]$.
