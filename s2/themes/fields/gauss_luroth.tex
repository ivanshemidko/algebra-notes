...
\begin{stmt}
	...
	\begin{proof}
		Пусть $f = g \cdot h$ в $Q[t]$, где $g, h \in Q[t]$. Запишем $f = c(f)\tilde{f}$, $g = c(g)\tilde{g} $, $h = c(h)\tilde{h}$.

		Тогда $c(f)\tilde{f} = c(g)\,c(h)\,\tilde{g}\tilde{h}$. Так как $\tilde{f}$ и $\tilde{g}\tilde{h}$ --- примитивные,

        имеем $c(f) = c(g)\,c(h)$ (с точностью до обратимого). Следовательно, $\tilde{f} = \tilde{g}\tilde{h}$, и это разложение в $R[t]$.
...
\begin{cor}
	...
	\begin{proof}
		Пусть $q = \frac{a}{b} \in \mathbb{Q}$ --- корень $f(t)$, $(a, b) = 1$. Тогда $f(t) = (t - q) \cdot g(t)$ в $\mathbb{Q}[t]$.

		Домножим: $f(t) = \left(\frac{1}{b}\right) g(t) \cdot (bt - a)$. Многочлен $bt - a$ примитивен и делит $f(t)$ в $\mathbb{Q}[t]$.
