...
\begin{definition}
    ...
    \begin{enumerate}
        ...
        \item $B$ называется \emph{знакопеременной}, если $B(v, v) = 0$ для любого $v \in V$.
        \item $B$ называется \emph{кососимметричной (антисимметричной)}, если $B(v, u) = -B(u, v)$ для любых $v, u \in V$.
    \end{enumerate}
\end{definition}


\begin{stmt}
    Знакопеременность влечёт кососимметричность (антисимметричность). Обратное верно тогда и только тогда,
    когда $\mathrm{char}\, k \neq 2$.
    \begin{proof}
        \textbf{Знакопеременность $\Rightarrow$ кососимметричность:}
        так как $B(u+v, u+v) = 0$, раскрываем:
        \[
            0 = B(u,u) + B(u,v) + B(v,u) + B(v,v) = B(u,v) + B(v,u),
        \]
        откуда $B(u,v) = -B(v,u)$. Это верно при любой характеристике.
        \textbf{Кососимметричность $\Rightarrow$ знакопеременность} (при $\mathrm{char}\, k \neq 2$):
        подставляем $u = v$:
        \[
...
\begin{stmt}
    ~\begin{enumerate}
        ...
        \[ B(v_i, v_j) = B(v_j, v_i). \]
        \item Если $B$ знакопеременна, $G_{\mathbf{v}}$ кососимметрична (и нули на диагонали)
            $G_{\mathbf{v}}^T = -G_{\mathbf{v}}$: \[ B(v_i, v_j) = -B(v_j, v_i). \]
        \item Если $B$ кососимметрична (антисимметрична), то по определению
            $B(v_i,v_j) = -B(v_j,v_i)$ для всех $i,j$, то есть $G_{\mathbf{v}}^T = -G_{\mathbf{v}}$ ---
            это верно при любой характеристике.

            При $\mathrm{char}\, k \neq 2$: подставляя $i=j$, получаем $2B(v_i,v_i)=0$, откуда
            $B(v_i,v_i)=0$ --- значит, $B$ автоматически знакопеременна (совпадает с условием
            из пункта 2), и на диагонали $G_{\mathbf{v}}$ стоят нули.

            При $\mathrm{char}\, k = 2$: равенство $G_{\mathbf{v}}^T=-G_{\mathbf{v}}$ равносильно
            $G_{\mathbf{v}}^T=G_{\mathbf{v}}$ (так как $-1=1$), то есть кососимметричные матрицы
            в этом случае совпадают с симметричными как множество матриц; при этом диагональные
            элементы $g_{ii}$ обнулять не обязаны (условие $2g_{ii}=0$ выполнено тождественно).
